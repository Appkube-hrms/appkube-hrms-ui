...
export const createUser = createAsyncThunk(
  ...
  async (data, { rejectWithValue }) => {
    try {
      ...
      console.log("post api hit", hrDetails);

      return hrDetails.data;
    } catch (error) {
      console.log("error post api hit", error);
      console.log("error of the axios", error.message);
      console.log("error of the axios", error.response.status);
  ...
  async (data, { rejectWithValue }) => {
    try {
      const response = await axios.put(
        "https://bwppdwpoab.execute-api.us-east-1.amazonaws.com/dev/organization",
        data,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
      ...
      console.log("post api hit", response);
      // return response.data;
    } catch (error) {
      console.log("error post api hit", error);
      console.log("error of the axios", error.message);
...
export const { setPersonalData, setCompanyData } =
